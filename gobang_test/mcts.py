...
def expansion(node, vacuity, path, board):

    waiting = set(map(lambda v: tuple(v), vacuity)) - set(map(lambda p: tuple(p.pos), path + node.child))
    # w = []
    # for i in waiting:
    
    w = set([i for i in waiting if Board.has_neighbor(board, i, 1)])

    if w:
        node.child.append(Node(choice(list(w))))
...
def mcts(queue, iteration=500):
    ...
    for i in range(iteration):
        ...
        if result == 0:
            node = expansion(node, vacuity, path, board)
